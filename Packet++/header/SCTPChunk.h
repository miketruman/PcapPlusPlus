#ifndef PACKETPP_SCTP_DATA
#define PACKETPP_SCTP_DATA
...
namespace pcpp
{
	...
	public:
		 ...
		 */
		SCTPChunk(uint8_t* recordRawData)
		{
		 ...
		 * @param[in] other The SCTPChunk instance to copy from
		 */
		 ...
		 * which means that after calling it both the old and the new instance will point to the same TLV raw data
		 * @param[in] other The SCTPChunk instance to assign
		 */
		 ...
		 */
		bool operator==(const SCTPChunk& rhs) const
		{
			...
			if (isNull() || ((SCTPChunk&)rhs).isNull())
				return false;
	...
	/**
	 * @class SCTPChunkReader
	 * A class for reading TLV records data out of a byte stream. This class contains helper methods for retrieving and
	 * counting TLV records. This is a template class that expects template argument class derived from SCTPChunk.
	 */
	template<typename SCTPChunkType>
	class SCTPChunkReader
	{
	private:
	...
	public:
		 ...
		 */
		SCTPChunkReader() { m_RecordCount = (size_t)-1; }

		/**
		 * A default copy c'tor for this class
		 */
		SCTPChunkReader(const SCTPChunkReader& other)
		{
		 ...
		 */
		virtual ~SCTPChunkReader() { }

		/**
		 * Overload of the assignment operator for this class
		 * @param[in] other The SCTPChunkReader instance to assign
		 */
		SCTPChunkReader& operator=(const SCTPChunkReader& other)
		{
			m_RecordCount = other.m_RecordCount;
			return *this;
		}
		 ...
		 * @return An instance of type SCTPChunkType that contains the first TLV record. If tlvDataBasePtr is NULL or
		 * tlvDataLen is zero the returned SCTPChunkType instance will be logically NULL, meaning SCTPChunkType.isNull() will
		 * return true
		 */
		SCTPChunkType getFirstSCTPChunk(uint8_t* tlvDataBasePtr, size_t tlvDataLen) const
		{
			...
			SCTPChunkType resRec(tlvDataBasePtr); // for NRVO optimization
		 ...
		 * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
		 * @param[in] tlvDataLen The TLV data byte stream length
		 * @return An instance of type SCTPChunkType that wraps the record following the record given as input. If the
		 * input record.isNull() is true or if the next record is out of bounds of the byte stream, a logical NULL instance
		 * of SCTPChunkType will be returned, meaning SCTPChunkType.isNull() will return true
		 */
		SCTPChunkType getNextSCTPChunk(SCTPChunkType& record, uint8_t* tlvDataBasePtr, size_t tlvDataLen) const
		{
			SCTPChunkType resRec(NULL); // for NRVO optimization
		 ...
		 * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
		 * @param[in] tlvDataLen The TLV data byte stream length
		 * @return An instance of type SCTPChunkType that contains the result record. If record was not found a logical
		 * NULL instance of SCTPChunkType will be returned, meaning SCTPChunkType.isNull() will return true
		 */
		SCTPChunkType getSCTPChunk(uint8_t recordType, uint8_t* tlvDataBasePtr, size_t tlvDataLen) const
		{
			SCTPChunkType curRec = getFirstSCTPChunk(tlvDataBasePtr, tlvDataLen);
			while (!curRec.isNull())
			{
				if (curRec.getType() == recordType)
					return curRec;

				curRec = getNextSCTPChunk(curRec, tlvDataBasePtr, tlvDataLen);
			}
		 ...
		 * to call changeSCTPChunkCount() with the record count change
		 * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
		 * @param[in] tlvDataLen The TLV data byte stream length
		 * @return The TLV record count
		 */
		size_t getSCTPChunkCount(uint8_t* tlvDataBasePtr, size_t tlvDataLen) const
		{
		...
		/**
		 * As described in getSCTPChunkCount(), the TLV record count is being cached for efficiency purposes. So if the
		 ...
		 */
		void changeSCTPChunkCount(int changedBy) { if (m_RecordCount != (size_t)-1) m_RecordCount += changedBy; }
	};


	/**
	 * @class SCTPChunkBuilder
	 * A base class for building Type-Length-Value (TLV) records. This builder receives the record parameters in its c'tor,
	 * builds the record raw buffer and provides a method to build a SCTPChunk object out of it. Please notice this is
	 * a base class that lacks the capability of actually building SCTPChunk objects and also cannot be instantiated. The
	...
	protected:
		...
		SCTPChunkBuilder(uint8_t recType, const uint8_t* recValue, uint8_t recValueLen);

		SCTPChunkBuilder(uint8_t recType, uint8_t recValue);

		SCTPChunkBuilder(uint8_t recType, uint16_t recValue);

		SCTPChunkBuilder(uint8_t recType, uint32_t recValue);

		SCTPChunkBuilder(uint8_t recType, const IPv4Address& recValue);

		SCTPChunkBuilder(uint8_t recType, const std::string& recValue);

		SCTPChunkBuilder(const SCTPChunkBuilder& other);

		SCTPChunkBuilder& operator=(const SCTPChunkBuilder& other);

		virtual ~SCTPChunkBuilder();
	...
	private:
		...
		void copyData(const SCTPChunkBuilder& other);
	};
}
